...
function Glass({ children, className = "", hover = true }: { children: ReactNode; className?: string; hover?: boolean }) {
  return (
    <motion.div
      variants={fadeUp}
      whileHover={hover ? { y: -6, scale: 1.02, transition: { duration: 0.25 } } : undefined}
...
function Navbar() {
  ...
  return (
    ...
    >
      ...
      {mobileOpen && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="md:hidden bg-black/60 backdrop-blur-2xl border-t border-white/[0.06] px-6 py-4 space-y-3">
          {["Platform", "Marketplace", "Enterprise", "Logistics", "Payments"].map((item) => (
...
function HeroSection() {
  ...
  return (
    <section ref={ref} className="relative h-screen overflow-hidden">
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 h-full flex flex-col items-center justify-center px-6">
        <div className="max-w-4xl mx-auto text-center -mt-[5vh]">
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, delay: 0.3, ease: [0.16, 1, 0.3, 1] }}>
            <motion.div
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white tracking-tight leading-[0.95]">
              <motion.span initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.8 }} className="block">Where Supply</motion.span>
              <motion.span initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75, duration: 0.8 }} className="block text-gradient mt-1">Meets Demand</motion.span>
              <motion.span initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.8 }} className="block text-white/40 text-3xl sm:text-4xl md:text-5xl mt-2 font-light">Into Infinity.</motion.span>
            </h1>

            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }} className="text-base md:text-lg text-white/60 mt-6 max-w-xl mx-auto leading-relaxed">
        ...
        {/* HUD Stats */}
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.6, duration: 0.7 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-3xl px-6">
    ...
    <section id="platform" className="py-32 px-6 relative z-10">
      <div className="max-w-7xl mx-auto">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="text-center mb-16">
          ...
          <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mt-4 tracking-tight">
            Everything. <span className="text-gradient">Connected.</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-lg text-white/50 mt-6 max-w-2xl mx-auto">
            A living ecosystem where every module feeds into the next — an integrated nervous system for the world's economy.
          </motion.p>
        </motion.div>

        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-50px" }} variants={stagger} className="relative w-full aspect-[16/10] max-h-[600px]">
          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            {connections.map((c, i) => (
              <motion.line key={i} x1={np[c.from].x} y1={np[c.from].y + 5} x2={np[c.to].x} y2={np[c.to].y}
                stroke="url(#lineGrad)" strokeWidth="0.15"
                initial={{ pathLength: 0, opacity: 0 }} whileInView={{ pathLength: 1, opacity: 1 }}
                viewport={{ once: true }} transition={{ duration: 1.2, delay: 0.3 + i * 0.1, ease: "easeInOut" }}
              />
            ))}
            {connections.map((c, i) => (
              <motion.circle key={`p-${i}`} r="0.4" fill="hsl(var(--primary))"
                initial={{ opacity: 0 }} whileInView={{ opacity: [0, 1, 1, 0] }}
                viewport={{ once: true }} transition={{ duration: 2.5, delay: 1.5 + i * 0.2, repeat: Infinity, repeatDelay: 3 }}
              >
          ...
          {nodes.map((node, i) => (
            <motion.div key={node.id}
              ...
              viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.15 * i, type: "spring", stiffness: 200 }}
              whileHover={{ scale: 1.12, zIndex: 20 }}
              className="absolute -translate-x-1/2 -translate-y-1/2 group cursor-pointer"
              style={{ left: node.x, top: node.y }}
            >
              <motion.div className="absolute inset-0 rounded-2xl bg-primary/20 blur-xl"
      ...
      <div className="max-w-7xl mx-auto">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl font-bold text-white mt-4 tracking-tight leading-[1.1]">
              From Raw Earth<br /><span className="text-gradient">To Final Product.</span>
            </motion.h2>
            <motion.p variants={fadeUp} className="text-lg text-white/50 mt-6 leading-relaxed">
    ...
    <section id="logistics" className="py-32 px-6 relative z-10">
      <div className="max-w-7xl mx-auto">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="text-center mb-20">
          ...
          <motion.p variants={fadeUp} className="text-lg text-white/50 mt-6 max-w-xl mx-auto">
