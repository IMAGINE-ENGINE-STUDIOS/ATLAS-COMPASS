...
// With cargo categories, vessel names, and simulated traffic data
...
export type CargoRoute = {
  ...
  vessels: Vessel[];
  distance: string; // nautical miles or km
  transitTime: string;
};

// Vessel name generators
const SHIP_NAMES = [
  "MSC Oscar","Ever Given","CSCL Globe","CMA CGM Marco Polo","Maersk Mc-Kinney","OOCL Hong Kong",
  "MOL Triumph","Cosco Shipping Universe","HMM Algeciras","ONE Stork","Hapag-Lloyd Berlin",
  "Yang Ming Warranty","ZIM Antwerp","Wan Hai Lines 522","PIL Kota Pekarang","Evergreen Triton",
  "Pacific Basin Vinson","Star Bulk Oasis","Golden Ocean Frontline","Torm Plata","Euronav Nautilus",
  "Teekay Torben","Flex LNG Endeavour","Golar LNG Partners","Höegh Autoliners","K Line Fuji",
  "NYK Vega","Stena Bulk Impero","Scorpio Tankers SCA","Frontline Ltd Eagle","Minerva Gloria",
  "BW LPG Rainbow","Dorian LPG Corsair","Navigator Holdings Aries","Ardmore Seafox",
  "Navios Maritime Star","Diana Shipping Elpis","Genco Shipping Claudius","Eagle Bulk Grebe",
  "Pacific International Kuma","Oldendorff Johanna","Berge Bulk Everest","Pan Ocean Serenity",
];
const PLANE_NAMES = [
  "Atlas Air 747-8F","Cargolux 747-400F","FedEx MD-11F","UPS 767-300F","DHL 777F","Cathay Pacific Cargo 747",
  "Emirates SkyCargo 777F","Qatar Airways Cargo A330F","Korean Air Cargo 747-8F","Nippon Cargo Airlines 747F",
  "China Airlines Cargo 747","AirBridgeCargo 747-8F","Silk Way Airlines 747","Turkish Cargo A330F",
  "Lufthansa Cargo MD-11F","Singapore Airlines Cargo 747","ANA Cargo 777F","Air China Cargo 777F",
  "Asiana Cargo 747-400F","EVA Air Cargo MD-11F","Ethiopian Airlines Cargo 777F",
  "Volga-Dnepr An-124","Antonov Airlines An-225","Kalitta Air 747-400F","Western Global 747-400F",
  "Polar Air Cargo 747-8F","Southern Air 777F","Air Hong Kong A330F","Centurion Cargo MD-11F",
];
const FLAGS = ["🇵🇦","🇱🇷","🇲🇭","🇭🇰","🇸🇬","🇬🇧","🇩🇪","🇯🇵","🇰🇷","🇳🇴","🇬🇷","🇨🇳","🇺🇸","🇩🇰","🇧🇸","🇨🇾","🇲🇹","🇮🇹"];
const OPERATORS = ["Maersk","MSC","CMA CGM","COSCO","Hapag-Lloyd","ONE","Evergreen","HMM","Yang Ming","ZIM","PIL","Wan Hai","Euronav","Teekay","Frontline","BW","TORM","Scorpio"];
const AIR_OPS = ["FedEx Express","UPS Airlines","DHL Aviation","Cargolux","Atlas Air","Emirates SkyCargo","Qatar Airways Cargo","Korean Air Cargo","Cathay Pacific Cargo","Nippon Cargo","Singapore Airlines Cargo","Turkish Cargo","Lufthansa Cargo","Air China Cargo","Ethiopian Cargo"];

function genVessels(count: number, type: "maritime"|"air", category: CargoCategory): Vessel[] {
  const names = type === "air" ? PLANE_NAMES : SHIP_NAMES;
  const ops = type === "air" ? AIR_OPS : OPERATORS;
  return Array.from({ length: count }, (_, i) => ({
    id: `v-${type}-${category}-${i}-${Math.random().toString(36).slice(2,6)}`,
    name: names[Math.floor(Math.random() * names.length)],
    flag: type === "air" ? "✈️" : FLAGS[Math.floor(Math.random() * FLAGS.length)],
    category,
    speed: type === "air" ? 800 + Math.floor(Math.random() * 120) : 12 + Math.floor(Math.random() * 14),
    progress: Math.random(),
    tonnage: type === "air" ? `${50 + Math.floor(Math.random()*80)}t` : `${20000 + Math.floor(Math.random()*180000)} DWT`,
    built: 2008 + Math.floor(Math.random() * 17),
    operator: ops[Math.floor(Math.random() * ops.length)],
    imo: type === "maritime" ? `IMO ${9000000+Math.floor(Math.random()*999999)}` : undefined,
  }));
}
...
export const MARITIME_ROUTES: CargoRoute[] = [
  ...
  { id:"m1",name:"Shanghai → Los Angeles",type:"maritime",color:"#3b82f6",category:"container",distance:"6,252 nm",transitTime:"14-16 days",
    ...
    vessels:genVessels(6,"maritime","container"),
  },
  { id:"m2",name:"Busan → Long Beach",type:"maritime",color:"#60a5fa",category:"container",distance:"5,887 nm",transitTime:"13-15 days",
    ...
    vessels:genVessels(5,"maritime","container"),
  },
  { id:"m3",name:"Tokyo → Seattle",type:"maritime",color:"#93c5fd",category:"roro",distance:"4,787 nm",transitTime:"10-12 days",
    ...
    vessels:genVessels(4,"maritime","roro"),
  },
  ...
  { id:"m6",name:"Houston → Rotterdam",type:"maritime",color:"#d97706",category:"oil",distance:"5,038 nm",transitTime:"14-16 days",
    ...
    vessels:genVessels(4,"maritime","oil"),
  },
  { id:"m7",name:"Hamburg → Savannah",type:"maritime",color:"#fbbf24",category:"container",distance:"4,348 nm",transitTime:"11-13 days",
    ...
    vessels:genVessels(3,"maritime","container"),
  },
  ...
  { id:"m9",name:"Singapore → Rotterdam (Suez)",type:"maritime",color:"#ef4444",category:"container",distance:"8,288 nm",transitTime:"21-25 days",
    ...
    vessels:genVessels(8,"maritime","container"),
  },
  { id:"m10",name:"Shanghai → Hamburg (Suez)",type:"maritime",color:"#dc2626",category:"container",distance:"10,594 nm",transitTime:"28-32 days",
    ...
    vessels:genVessels(7,"maritime","container"),
  },
  ...
  { id:"m11",name:"Ras Tanura → Chiba (Oil)",type:"maritime",color:"#f59e0b",category:"oil",distance:"6,590 nm",transitTime:"18-22 days",
    ...
    vessels:genVessels(6,"maritime","oil"),
  },
  { id:"m12",name:"Qatar → Incheon (LNG)",type:"maritime",color:"#06b6d4",category:"lng",distance:"7,100 nm",transitTime:"20-24 days",
    ...
    vessels:genVessels(5,"maritime","lng"),
  },
  ...
  { id:"m16",name:"Busan → Kaohsiung",type:"maritime",color:"#2dd4bf",category:"bulk",distance:"910 nm",transitTime:"2-3 days",
    ...
    vessels:genVessels(4,"maritime","bulk"),
  },
  // ── Cape of Good Hope ──
  { id:"m17",name:"Singapore → Cape Town",type:"maritime",color:"#f97316",category:"oil",distance:"5,935 nm",transitTime:"16-19 days",
    ...
    vessels:genVessels(3,"maritime","oil"),
  },
  { id:"m18",name:"Cape Town → Rotterdam",type:"maritime",color:"#ea580c",category:"bulk",distance:"6,233 nm",transitTime:"17-20 days",
    ...
    vessels:genVessels(3,"maritime","bulk"),
  },
  // ── South America ──
  ...
  { id:"m20",name:"New York → LA (Panama)",type:"maritime",color:"#06b6d4",category:"container",distance:"4,897 nm",transitTime:"13-15 days",
    ...
    vessels:genVessels(4,"maritime","container"),
  },
  // ── Mediterranean ──
  ...
  { id:"m22",name:"Algeciras → Port Said",type:"maritime",color:"#be123c",category:"chemical",distance:"2,100 nm",transitTime:"5-7 days",
    ...
    vessels:genVessels(4,"maritime","chemical"),
  },
  // ── Africa & Australia ──
  ...
  { id:"m25",name:"Sydney → Shanghai",type:"maritime",color:"#0284c7",category:"lng",distance:"4,990 nm",transitTime:"13-15 days",
    ...
    vessels:genVessels(3,"maritime","lng"),
  },
  // ── Northern Sea Route ──
  { id:"m26",name:"Murmansk → Busan (Arctic)",type:"maritime",color:"#64748b",category:"lng",distance:"7,200 nm",transitTime:"20-25 days",
    ...
    vessels:genVessels(2,"maritime","lng"),
  },
  { id:"m27",name:"Chennai → Jeddah",type:"maritime",color:"#8b5cf6",category:"general",distance:"2,580 nm",transitTime:"7-9 days",
    ...
    vessels:genVessels(3,"maritime","general"),
  },
];
...
export const AIR_CARGO_ROUTES: CargoRoute[] = [
  { id:"a1",name:"Hong Kong → Los Angeles",type:"air",color:"#f472b6",category:"air-cargo",distance:"11,650 km",transitTime:"13h",
    ...
    vessels:genVessels(3,"air","air-cargo"),
  },
  { id:"a2",name:"Shanghai → Chicago (Polar)",type:"air",color:"#fb7185",category:"air-cargo",distance:"11,023 km",transitTime:"14h",
    ...
    vessels:genVessels(2,"air","air-cargo"),
  },
  { id:"a3",name:"Incheon → Anchorage → Memphis",type:"air",color:"#e879f9",category:"air-express",distance:"12,400 km",transitTime:"16h",
    ...
    vessels:genVessels(3,"air","air-express"),
  },
  { id:"a4",name:"London → New York (JFK)",type:"air",color:"#facc15",category:"air-cargo",distance:"5,570 km",transitTime:"7h",
    ...
    vessels:genVessels(4,"air","air-cargo"),
  },
  ...
  { id:"a11",name:"Memphis → Cologne (FedEx)",type:"air",color:"#67e8f9",category:"air-express",distance:"7,975 km",transitTime:"10h",
    ...
    vessels:genVessels(2,"air","air-express"),
  },
...
export const ALL_VESSELS = ALL_CARGO_ROUTES.flatMap(r => r.vessels.map(v => ({ ...v, routeId: r.id, routeName: r.name, routeType: r.type, routeColor: r.color })));
export const MARITIME_VESSEL_COUNT = MARITIME_ROUTES.reduce((s, r) => s + r.vessels.length, 0);
export const AIR_VESSEL_COUNT = AIR_CARGO_ROUTES.reduce((s, r) => s + r.vessels.length, 0);
