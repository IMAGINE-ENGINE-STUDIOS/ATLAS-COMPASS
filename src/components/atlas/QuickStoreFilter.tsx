  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="relative flex items-center gap-1.5 pl-1.5.5 pr-2.5.5 py-1 rounded-full backdrop-blur-xl border transition-all select-none touch-none"
        style={open ? {
      ...
      >
        ...
        >
          {current?.icon ?? <Store className="w-2.5.5 h-2.5.5" />}
        </span>
        ...
        {value && value !== "all" && (
          <span className="absolute -top-0.5 -right-0.5 w-1.5.5 h-1.5.5 rounded-full bg-emerald-400 border-2 border-black/70" />
        )}
      </button>
      ...
      {/* Label chip */}
      {open && current && (
        <div className="px-1.5.5 py-1 rounded-full bg-black/70 backdrop-blur-xl border border-white/[0.08] text-[10px] font-mono text-white/85 animate-fade-in whitespace-nowrap">
          {current.label}
        </div>
      )}
    </div>
