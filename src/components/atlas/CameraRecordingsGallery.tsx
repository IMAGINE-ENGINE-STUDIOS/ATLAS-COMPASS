...
export default function CameraRecordingsGallery({ open, onClose }: Props) {
  ...
  return (
  <>
    <div className="absolute top-20 left-4 z-40 w-[calc(100vw-2rem)] max-w-[357px]">
      <div className="rounded-xl bg-black/85 backdrop-blur-2xl border border-white/[0.1] shadow-2xl overflow-hidden flex flex-col max-h-[calc(100dvh-7rem)]">
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-white/[0.06]">
          <div className="w-6 h-6 rounded-md bg-red-500/15 border border-red-500/30 flex items-center justify-center">
            <Film className="w-2.5.5 h-2.5.5 text-red-400" />
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-1.5 space-y-1">
          {items.length === 0 && (
            <div className="text-center py-7 text-xs text-white/60">
              No recordings yet. Open a camera and tap <span className="text-red-300 font-bold">Record</span>.
            </div>
          )}
          {items.map(rec => (
            <div key={rec.id} className="flex items-center gap-1.5 p-1.5 rounded-lg bg-black/40 border border-white/[0.04]">
              <div className="w-12 h-9 rounded-md overflow-hidden bg-black/70 border border-white/[0.06] shrink-0 relative">
                ...
                {rec.state === "paused" && (
                  <span className="absolute top-0.5 left-0.5 w-1 h-1 rounded-full bg-amber-400" />
                )}
              </div>
              <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-0.5 shrink-0">
                {rec.state === "recording" && (
                  <button onClick={() => cameraRecordings.pause(rec.id)} title="Pause"
                    className="p-1 rounded-md text-amber-300 hover:bg-white/[0.06] transition">
                    <Pause className="w-2.5.5 h-2.5.5" />
                  </button>
                )}
                {rec.state === "paused" && (
                  <button onClick={() => cameraRecordings.resume(rec.id)} title="Resume"
                    className="p-1 rounded-md text-emerald-300 hover:bg-white/[0.06] transition">
                    <Play className="w-2.5.5 h-2.5.5" />
                  </button>
                )}
                {rec.state !== "finished" && (
                  <button onClick={() => cameraRecordings.stop(rec.id)} title="Stop & save"
                    className="p-1 rounded-md text-red-300 hover:bg-white/[0.06] transition">
                    <Square className="w-2.5.5 h-2.5.5" />
                  </button>
                )}
                {rec.state === "finished" && rec.blobUrl && (
                  <button onClick={() => setPlaying(rec)} title="Watch in-app"
                    className="p-1 rounded-md text-white/80 hover:bg-white/[0.06] transition">
                    <Play className="w-2.5.5 h-2.5.5" />
                  </button>
                )}
                {rec.state === "finished" && (
                  <button onClick={() => cameraRecordings.download(rec.id)} title="Download"
                    className="p-1 rounded-md text-white/80 hover:bg-white/[0.06] transition">
                    <Download className="w-2.5.5 h-2.5.5" />
                  </button>
                )}
                <button onClick={() => cameraRecordings.remove(rec.id)} title="Delete"
                  className="p-1 rounded-md text-white/50 hover:text-red-300 hover:bg-white/[0.06] transition">
    ...
    {playing && playing.blobUrl && (
      <div
        className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl flex items-center justify-center p-3"
        onClick={() => setPlaying(null)}
      >
        <div
          className="relative w-full max-w-5xl rounded-xl overflow-hidden border border-white/10 bg-black shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-white/10 bg-black/70">
            ...
            <button
              ...
              title="Download"
              className="p-1 rounded-md text-white/80 hover:bg-white/10"
            >
              <Download className="w-2.5.5 h-2.5.5" />
            </button>
            <button
