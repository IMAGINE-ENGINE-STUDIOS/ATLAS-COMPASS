      ...
      <div className="relative backdrop-blur-2xl bg-black/75 border border-white/[0.1] rounded-2xl overflow-hidden shadow-[0_16px_60px_rgba(0,0,0,0.5)]">
        ...
        <div className="relative z-10 p-4">
          ...
          <div className="flex items-start gap-3 mb-3">
            {/* Product icon */}
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-violet-500/20 to-fuchsia-500/10 border border-white/[0.1] flex items-center justify-center text-2xl shrink-0">
              {product.emoji || "🛍️"}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-md bg-violet-500/15 text-violet-300 border border-violet-500/20">
                  {product.category}
                </span>
                {product.stock > 0 ? (
          ...
          <div className="grid grid-cols-2 gap-1.5 mt-3">
            <button
              ...
              className="flex items-center justify-center gap-1 py-2 rounded-xl text-[11px] font-bold bg-violet-500/15 text-violet-300 border border-violet-500/20 hover:bg-violet-500/25 transition-all truncate px-2"
            >
              <ShoppingBag className="w-3.5 h-3.5 shrink-0" /> Buy Now
            </button>
            {onDirections && (
              <button
                onClick={() => onDirections(product)}
                className="flex items-center justify-center gap-1 py-2 rounded-xl text-[11px] font-bold bg-blue-500/15 text-blue-400 border border-blue-500/20 hover:bg-blue-500/25 transition-all truncate px-2"
              >
            ...
            {onDelivery && (
              <button
                ...
                className="flex items-center justify-center gap-1 py-2 rounded-xl text-[11px] font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 hover:bg-cyan-500/20 transition-all truncate px-2"
              >
                <Truck className="w-3.5 h-3.5 shrink-0" /> Deliver
              </button>
            )}
            <button
              onClick={() => {
